...
void processCommand(uint8_t* data, size_t length) {
  ...
  if (enableLogging) {
    Serial.printf("处理命令: 0x%02X (%s), 数据长度: %d\n", cmdId,
                  getCommandName(cmdId), payloadLength);
  }
...
// 发送命令响应
void sendResponse(uint8_t cmdId, uint8_t statusCode, uint8_t* payload,
  ...
  uint8_t responseLength =
      3 + payloadLength;  // 命令ID + 状态码 + 负载长度 + 负载
  ...
  if (enableLogging) {
    Serial.printf("发送响应: 命令=0x%02X, 状态=%s(0x%02X), 长度=%d\n", cmdId,
                  getStatusName(statusCode), statusCode, payloadLength);
  }
}